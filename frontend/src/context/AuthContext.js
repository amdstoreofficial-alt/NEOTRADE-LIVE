...
export const AuthProvider = ({ children }) => {
  ...
  const [token, setToken] = useState(localStorage.getItem('neotrade_token') || localStorage.getItem('orbitrade_token'));

  useEffect(() => {
    const initAuth = async () => {
      const storedToken = localStorage.getItem('neotrade_token') || localStorage.getItem('orbitrade_token');
      if (storedToken) {
        try {
