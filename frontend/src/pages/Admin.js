...
const AdminPanel = () => {
  ...
  // Commission structure state (direct-only)
  const [commissionForm, setCommissionForm] = useState({ direct_percent: 5 });
        ...
        {activeTab === 'commissions' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-2xl">
            <div className="bg-white/[0.02] rounded-xl border border-white/[0.06] p-5">
              ...
              <p className="text-[11px] text-gray-400 mb-1">Direct referrals only — single tier.</p>
              <p className="text-[10px] text-gray-500 mb-4">Users earn this commission rate on every approved deposit made by their direct referrals. No indirect/multi-level or profit-sharing commissions.</p>

              <div className="space-y-4">
                <div>
                  <label className="text-[10px] text-gray-500 uppercase tracking-wider mb-1 block">Direct Commission %</label>
                  <input type="number" value={commissionForm.direct_percent} onChange={(e) => setCommissionForm({...commissionForm, direct_percent: parseFloat(e.target.value) || 0})}
                    className="w-full bg-black/30 border border-white/10 rounded-lg px-3 py-2 text-sm text-white font-mono focus:outline-none focus:border-brand" step="0.1" min="0" max="100" data-testid="direct-percent" />
                  <p className="text-[9px] text-gray-600 mt-1">Percentage paid to the direct referrer on each approved deposit (e.g. 5% on a $100 deposit = $5 commission).</p>
                </div>

                <button onClick={saveCommissionStructure}
                  className="w-full py-3 rounded-xl bg-gradient-brand text-white font-semibold text-sm transition-all hover:shadow-[0_0_20px_rgba(139,92,246,0.45)]"
