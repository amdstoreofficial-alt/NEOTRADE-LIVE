...
import { Copy, Users, DollarSign, Check, ShieldAlert, Share2, ArrowUpRight, Clock, PiggyBank } from 'lucide-react';
...
const Affiliate = () => {
      ...
      <div className="min-h-screen bg-app" data-testid="affiliate-page">
        ...
        <main className="pt-16 pb-8 px-3 sm:px-6 max-w-4xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <button onClick={() => navigate('/dashboard')} className="px-6 py-2.5 rounded-xl bg-gradient-brand text-white text-sm font-medium" data-testid="back-to-dashboard">Back to Dashboard</button>
          </motion.div>
        </main>
      </div>
    );
  ...
  const directPercent = commissionStructure?.direct_percent ?? 5;
    ...
    <div className="min-h-screen bg-app" data-testid="affiliate-page">
      ...
      <main className="pt-16 pb-8 px-3 sm:px-6 max-w-6xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mt-6">
          ...
          <p className="text-sm text-gray-500 mb-6">Earn {directPercent}% direct commission on every deposit made by users you refer</p>

          {/* Stats Grid — 3 cards (direct referrals only) */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-3 mb-6">
            <div className="bg-white/[0.02] rounded-xl border-l-2 border-electric/50 p-3 sm:p-4" data-testid="stat-total-referrals">
              <div className="flex items-center gap-2 mb-1">
                <Users className="w-4 h-4 text-electric" />
                <span className="text-[10px] text-gray-500 uppercase tracking-wider">Direct Referrals</span>
              </div>
              <div className="font-mono text-xl sm:text-2xl font-bold text-white">{stats?.total_referrals || 0}</div>
            </div>
            <div className="bg-white/[0.02] rounded-xl border-l-2 border-emerald-500/50 p-3 sm:p-4" data-testid="stat-total-earned">
              <div className="flex items-center gap-2 mb-1">
            ...
            <div className="bg-white/[0.02] rounded-xl border-l-2 border-brand/50 p-3 sm:p-4" data-testid="stat-commission-rate">
              <div className="flex items-center gap-2 mb-1">
                <PiggyBank className="w-4 h-4 text-brand" />
                <span className="text-[10px] text-gray-500 uppercase tracking-wider">Commission Rate</span>
              </div>
              <div className="font-mono text-xl sm:text-2xl font-bold text-brand">{directPercent}%</div>
            </div>
          </div>
          ...
          <div className="bg-white/[0.02] rounded-xl border border-white/[0.06] p-4 sm:p-5 mb-6">
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              ...
              <button onClick={copyLink}
                className={`px-4 py-2.5 rounded-xl font-semibold text-sm transition-all flex items-center justify-center gap-2 ${copied ? 'bg-emerald-500/20 text-emerald-400' : 'bg-gradient-brand text-white hover:shadow-[0_0_20px_rgba(139,92,246,0.4)]'}`}
              ...
              <div className="space-y-3">
                <div className="p-4 rounded-lg bg-emerald-500/5 border border-emerald-500/20">
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-sm text-white font-semibold">Direct Commission</span>
                    <span className="font-mono text-lg font-bold text-emerald-400">{directPercent}%</span>
                  </div>
                  <p className="text-[11px] text-gray-400">You earn <span className="text-emerald-400 font-semibold">{directPercent}%</span> commission on every approved deposit made by users who signed up using your referral link.</p>
                </div>
                <div className="p-3 rounded-lg bg-white/[0.02] border border-white/10">
                  <p className="text-[11px] text-gray-500 leading-relaxed">
                    <span className="text-white font-semibold">Single-tier only.</span> Commissions are paid on <span className="text-white">direct referrals</span> only — no multi-level or sub-referral earnings. Commission is credited to your real balance the moment a referral's deposit is approved.
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-4">
                <div className="flex gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-gray-500">Send your unique referral link to friends. When they sign up through it, they become your direct referral.</p>
                  </div>
                </div>
                <div className="flex gap-3">
                  ...
                  <div>
                    <h4 className="text-xs font-semibold text-white mb-0.5">They Deposit</h4>
                    <p className="text-[10px] text-gray-500">Every time your direct referral makes a deposit that's approved by admin, you instantly earn {directPercent}% commission.</p>
                  </div>
                </div>
                <div className="flex gap-3">
                  ...
                  <div>
                    <h4 className="text-xs font-semibold text-white mb-0.5">Get Paid Instantly</h4>
                    <p className="text-[10px] text-gray-500">Commission is credited to your real balance automatically — withdraw anytime, no minimums.</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white/[0.02] rounded-xl border border-white/[0.06] overflow-hidden mb-6">
            ...
            {!stats?.recent_commissions?.length ? (
              <div className="p-8 text-center text-gray-600 text-xs" data-testid="no-commissions">
              ...
              <div className="divide-y divide-white/[0.04]">
                {stats.recent_commissions.map((c, i) => (
                  <div key={i} className="px-4 py-3 flex items-center justify-between" data-testid={`commission-row-${i}`}>
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-emerald-500/10">
                        <ArrowUpRight className="w-4 h-4 text-emerald-400" />
                      </div>
                      <div>
                        <div className="text-xs font-semibold text-white">Direct Commission</div>
                        <div className="text-[10px] text-gray-600">From deposit of ${c.source_amount?.toFixed(2)}</div>
                      </div>
                    </div>
          ...
          <div className="bg-white/[0.02] rounded-xl border border-white/[0.06] overflow-hidden">
            <div className="px-4 py-3 border-b border-white/[0.04] flex justify-between items-center">
              <h3 className="text-sm font-bold text-white">Your Direct Referrals</h3>
              <span className="text-[10px] text-gray-600">{stats?.referrals?.length || 0} total</span>
            </div>
            {!stats?.referrals?.length ? (
              <div className="p-8 text-center text-gray-600 text-xs" data-testid="no-referrals">
              ...
              <div className="divide-y divide-white/[0.04]">
                {stats.referrals.map((r, i) => (
                  <div key={i} className="px-4 py-3 flex items-center justify-between" data-testid={`referral-row-${i}`}>
                    <div className="flex items-center gap-3">
