...
async def lifespan(app: FastAPI):
    ...
    logger.info("NEOTRADE Trading Platform started")
    yield
    
    client.close()
    logger.info("NEOTRADE Trading Platform shutdown")
...
async def seed_default_accounts():
    """Seed default admin and master user accounts (NEOTRADE)"""
    # Clean up any legacy ORBITRADE-branded accounts so new credentials take effect
    legacy_emails = ["admin@orbitrade.live", "masteruser@orbitrade.live"]
    for legacy in legacy_emails:
        await db.users.delete_one({"email": legacy})

    default_accounts = [
        {
            "_id": "admin_neotrade_001",
...
fastapi_app = FastAPI(title="NEOTRADE Trading Platform", version="1.0.0", lifespan=lifespan)
...
async def setup_2fa(request: Request):
    ...
    provisioning_uri = totp.provisioning_uri(user["email"], issuer_name="NEOTRADE Trading")
...
async def process_affiliate_commissions(user_id: str, amount: float, commission_type: str = "deposit"):
    """Process DIRECT affiliate commission (single-level) — ONLY on deposits.
    No indirect commissions. No revenue share on trading profits."""
    try:
        # Only deposit-based commissions are supported
        ...
        # Direct referrer only (no chain traversal)
        referrer = await db.users.find_one({"_id": user["referred_by"]})
        if not referrer:
            return

        # Get commission structure (direct_percent only)
        structure = await db.commission_structures.find_one({"active": True})
        direct_percent = (structure or {}).get("direct_percent", 5)
        commission_rate = direct_percent / 100
        ...
        logger.info(f"Direct affiliate commission: ${commission_amount:.2f} to {referrer['_id']} from deposit of ${amount:.2f}")
...
async def get_commission_structure():
    structure = await db.commission_structures.find_one({"active": True})
    if not structure:
        return {"direct_percent": 5}
    return {"direct_percent": structure.get("direct_percent", 5)}
...
async def chat_with_ai(request: Request, data: ChatMessage):
    ...
    try:
        ...
        chat = LlmChat(
            api_key=api_key,
            session_id=session_id,
            system_message="""You are NEOTRADE's AI Trading Assistant. You help users with:
...
async def get_admin_commission_structures(request: Request):
    ...
    if not structures:
        return [{"direct_percent": 5, "active": True}]
    return [{"direct_percent": s.get("direct_percent", 5), "active": s.get("active", False), "id": str(s.get("_id"))} for s in structures]
...
async def save_commission_structure(request: Request, data: dict = Body(...)):
    ...
    # Create new active structure (direct-only, single-level)
    structure = {
        "direct_percent": float(data.get("direct_percent", 5)),
        "active": True,
        "created_at": datetime.now(timezone.utc)
    }
    
    await db.commission_structures.insert_one(structure)
    return {"message": "Commission structure saved", "direct_percent": structure["direct_percent"]}
